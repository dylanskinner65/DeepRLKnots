...
class PolicyNetwork(nn.Module):
  def __init__(self, state_size, action_size):
    ...
    self.net = nn.Sequential(nn.Linear(state_size, hidden_size*4),
                             nn.ReLU(),
...
def knot_sampler(df):
    ...
    filtered = filtered[filtered.word.apply(lambda x: 4 <= len(x) <= 6)]
...
def ppo_main(gpu, args): 
  ...
  checkpoint = torch.load("training_files/knot_gpu_optim2.pt")
  ...
  writer = SummaryWriter(log_dir='runs/optim_euler')
  ...
  for epoch in range(epochs):
    ...
    for episode in range(env_samples):
      ...
      if done and cum_reward > -50:
        ...
        return_list.to_csv("Result_csv/results.csv")
    ...
    if correct_solved == 1:
        ...
        }, 'training_files/knot_gpu_optim2.pt')  # Changed to knot_gpu_12 because I am introducing a 12 crossing knot. 
